    ...
    <View style={[styles.container, { paddingTop: insets.top }]} testID="register-screen">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          <View style={styles.iconBadge}><Ionicons name="leaf" size={28} color={c.brand} /></View>
          ...
          <TextInput testID="register-name-input" value={name} onChangeText={setName}
            placeholder="Tu nombre" placeholderTextColor={c.muted} style={styles.input} />
          ...
          <TextInput testID="register-password-input" value={password} onChangeText={setPassword}
            placeholder="Mínimo 6 caracteres" placeholderTextColor={c.muted} secureTextEntry style={styles.input} />

          {error && <Text style={styles.error} testID="register-error">{error}</Text>}

          <Pressable testID="register-submit-button" onPress={submit} disabled={loading}
            style={({ pressed }) => [styles.cta, pressed && { opacity: 0.85 }]}>
            {loading ? <ActivityIndicator color={c.onBrandPrimary} /> : <Text style={styles.ctaText}>Crear cuenta</Text>}
          </Pressable>
...
const createStyles = (c: Palette) => StyleSheet.create({
  ...
  iconBadge: { width: 60, height: 60, borderRadius: radius.lg, backgroundColor: c.brandTertiary, alignItems: "center", justifyContent: "center", marginBottom: spacing.md },

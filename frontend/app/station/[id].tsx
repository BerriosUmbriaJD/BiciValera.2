...
export default function StationDetail() {
  ...
  return (
    <View style={styles.container} testID="station-detail">
      ...
      {loading ? (
        <ActivityIndicator size="large" color={colors.brand} style={{ marginTop: spacing["3xl"] }} />
      ) : (
        <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
          {station?.bikes?.length === 0 && (
            <Text style={styles.noBikes} testID="no-bikes">No hay bicicletas disponibles en esta estación.</Text>
          )}
          {station?.bikes?.map((b: any) => {
            ...
            return (
              <Pressable
                key={b.id}
                testID={`bike-option-${b.id}`}
                onPress={() => { Haptics.selectionAsync(); setSelected(b.id); }}
                style={[styles.bikeRow, active && styles.bikeRowActive]}
              >
                <View style={[styles.bikeIcon, { backgroundColor: (isElec ? colors.info : colors.brandSecondary) + "1A" }]}>
                  <Ionicons name={isElec ? "battery-charging" : "bicycle"} size={22} color={isElec ? colors.info : colors.brandSecondary} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.bikeCode}>Bici {b.code}</Text>
                  <Text style={styles.bikeType}>
                    {isElec ? `Eléctrica · ${b.battery}% batería` : "Mecánica"}
                  </Text>
                </View>
                <Ionicons
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + spacing.md }]}>
        ...
        >
          {starting ? <ActivityIndicator color={colors.onBrandPrimary} /> : (
            <Text style={styles.ctaText}>Desbloquear y comenzar</Text>
          )}
        </Pressable>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  handle: { width: 40, height: 5, borderRadius: 3, backgroundColor: colors.borderStrong, alignSelf: "center", marginTop: spacing.md },
  ...
  noBikes: { fontSize: 15, color: colors.muted, textAlign: "center", marginTop: spacing.xl },
  bikeRow: {
    flexDirection: "row", alignItems: "center", gap: spacing.md, backgroundColor: colors.surfaceSecondary,
    borderRadius: radius.md, padding: spacing.lg, borderWidth: 1.5, borderColor: colors.border,
  },
  bikeRowActive: { borderColor: colors.brand, backgroundColor: colors.brandTertiary },
  bikeIcon: { width: 44, height: 44, borderRadius: radius.sm, alignItems: "center", justifyContent: "center" },
  bikeCode: { fontSize: 16, fontWeight: "700", color: colors.onSurface },
  bikeType: { fontSize: 13, color: colors.muted, marginTop: 2 },
  error: { color: colors.error, fontSize: 14, textAlign: "center", paddingHorizontal: spacing.xl, marginBottom: spacing.sm },
  footer: { paddingHorizontal: spacing.xl, paddingTop: spacing.md, borderTopWidth: 1, borderTopColor: colors.divider, backgroundColor: colors.surface },
  cta: { backgroundColor: colors.brandPrimary, borderRadius: radius.md, paddingVertical: 17, alignItems: "center", ...shadow.card },

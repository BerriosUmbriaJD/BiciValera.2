...
export default function Unlock() {
  ...
  const pickAndStart = async () => {
    ...
    if (!station) {
      setError("No hay bicicletas disponibles en este momento");
      return;
    }
    setScanning(true);
    try {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <View style={[styles.sheet, { paddingBottom: insets.bottom + spacing.lg }]}>
          <Pressable
            ...
            onPress={pickAndStart}
            disabled={scanning}
            style={({ pressed }) => [styles.scanBtn, pressed && { opacity: 0.9 }]}
          >
            <Ionicons name="scan" size={20} color={colors.onBrandPrimary} />
            <Text style={styles.scanBtnText}>Simular escaneo</Text>
          </Pressable>

          <View style={styles.orRow}>
            <View style={styles.orLine} />
            <Text style={styles.orText}>o ingresa el código</Text>
            <View style={styles.orLine} />
          </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: colors.onSurfaceInverse, fontSize: 18, fontWeight: "700" },
  viewfinderWrap: { flex: 1, alignItems: "center", justifyContent: "center", gap: spacing.lg },
  viewfinder: {
    width: 240, height: 240, borderRadius: radius.lg, backgroundColor: "rgba(255,255,255,0.04)",
    alignItems: "center", justifyContent: "center", overflow: "hidden",
  },
  corner: { position: "absolute", width: 34, height: 34, borderColor: colors.brandSecondary },
  ...
  scanLine: { position: "absolute", left: 12, right: 12, height: 2, backgroundColor: colors.success, opacity: 0.8 },
  scanningOverlay: { ...StyleSheet.absoluteFillObject, alignItems: "center", justifyContent: "center", gap: spacing.md, backgroundColor: "rgba(19,28,21,0.6)" },
  scanningText: { color: colors.onSurfaceInverse, fontSize: 15, fontWeight: "600" },
  hint: { color: colors.onSurfaceInverse, opacity: 0.7, fontSize: 14, textAlign: "center", paddingHorizontal: spacing.xl },
  ...
  scanBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.sm, backgroundColor: colors.brandPrimary, borderRadius: radius.md, paddingVertical: 16, ...shadow.card },
  ...
  codeInput: { flex: 1, backgroundColor: colors.surfaceSecondary, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, paddingHorizontal: spacing.lg, paddingVertical: 14, fontSize: 16, color: colors.onSurface },
  codeBtn: { width: 52, borderRadius: radius.md, backgroundColor: colors.brandSecondary, alignItems: "center", justifyContent: "center" },
